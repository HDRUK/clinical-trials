...
FULL_TRIALS_DOWNLOAD_URL = "https://www.clinicaltrialsregister.eu/ctr-search/rest/download/full?query={}&country=gb&page={}&mode=current_page"
...
def create_list_of_trial_dicts(full_trial_data):
    ...
    for trial in split_full_trial_data_10:
        index_B = [
            i
            for i in trial
            if i.startswith("B.1.1 Name of Sponsor:")
        ...
        index_D_3 = [
            i
            for i in trial
            if i.startswith("D.IMP:") or i.startswith("D.8 Information on Placebo")
        ]
        ind_D_3 = []
        for i in index_D_3:
    ...
    for i in range(len(split_full_trial_data_10)):
        split_full_trial_data_11.append(
            dict(s.split(":", 1) for s in split_full_trial_data_10[i])
        )
...
def create_key_for_sections(split_full_trial_data):
    ...
    for trial in split_full_trial_data:
        ...
        Summary_subset = {
            ...
            and not key.startswith("D.")
            and not key.startswith("E.")
            and not key.startswith("F.")
            and not key.startswith("G.")
            and not key.startswith("N.")
            and not key.startswith("P.")
        }
    ...
    for i in range(len(A_subsets)):
        ...
        G_subsets[i] = {
            "G. Investigator Networks to be involved in the Trial": G_subsets[i]
        }
        N_subsets[i] = {
            "N. Review by the Competent Authority or Ethics Committee in the country concerned": N_subsets[
                i
            ]
        }
